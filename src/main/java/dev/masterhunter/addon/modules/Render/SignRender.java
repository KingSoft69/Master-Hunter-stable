...
public class SignRender extends Module {
    ...
    public SignRender() {
        super(Main.CATEGORY, "sign-render", "Renders sign text through walls with advanced clustering.");
    }
        ...
        while (!toCluster.isEmpty()) {
            ...
            while (iter.hasNext()) {
                ...
                if (distSq <= radiusSq) {
                    ...
                    clustered.add(other);
                    iter.remove(); // Remove clustered item to avoid redundant processing
                }
            }
    ...
    private String cleanSignText(String text) {
        ...
        // Remove Minecraft formatting codes (§ codes, & codes for all valid formatting chars including hex)
        text = text.replaceAll("§.", "");
        text = text.replaceAll("&[0-9a-fA-Fklmnorx]", "");

        // Handle JSON-style text
        if (text.contains("{\"") || text.contains("[\"")) {
            text = text.replaceAll("\\{\".*?\":\"(.*?)\".*?\\}", "$1");
            text = text.replaceAll("\\[\"(.*?)\"\\]", "$1");
        }

        // Clean up any remaining JSON artifacts (match content between braces that doesn't contain braces)
        text = text.replaceAll("\\{[^\\{\\}]*\\}", "");
